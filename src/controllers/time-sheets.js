...
const getAll = async(req, res) => {
    try {
        const allTimeSheets = await TimesheetModel.find({}).populate('tasks',{
            description:1,
            workedHours:1,
            date:1,
            _id:0
        }).populate('employeeId').populate('projectId');
        res.status(200).json({
...
const deleteTimesheet = async(req, res) => {
    try {
        const result = await TimesheetModel.findByIdAndDelete({ _id: req.params.id }).populate('tasks',).populate('employeeId').populate('projectId');;
        if (!result) {
